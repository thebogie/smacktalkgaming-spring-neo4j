package com.msg.smacktalkgaming.backend.domain.rating;
...
public class RatingCalculator {
	...
	/**
	 * 
	 * @param initVolatility
	 *            Initial volatility for new ratings
	 * @param tau
	...
	/**
	 * <p>
	 * Run through all players within a resultset and calculate their new
	 * ratings.
	 * </p>
	 * <p>
	 * Players within the resultset who did not compete during the rating period
	 * will have see their deviation increase (in line with Prof Glickman's
	 * paper).
	 * </p>
	 * <p>
	 * Note that this method will clear the results held in the association
	 * resultset.
	 * </p>
	 * 
	 * @param results
	...
	public void updateRatings(RatingPeriodResults results) {
		for (Rating player : results.getParticipants()) {
			if (results.getResults(player).size() > 0) {
				...
				// if a player does not compete during the rating period, then
				// only Step 6 applies.
				// the player's rating and volatility parameters remain the same
				// but deviation increases
				player.setWorkingRating(player.getGlicko2Rating());
				player.setWorkingRatingDeviation(
						calculateNewRD(player.getGlicko2RatingDeviation(), player.getVolatility()));
		...
		// lastly, clear the result set down in anticipation of the next rating
		// period
		results.clear();
	}
	...
	private void calculateNewRating(Rating player, List<Result> results) {
		...
		// step 5.2 - set the initial values of the iterative algorithm to come
		// in step 5.4
		double A = a;
		double B = 0.0;
		...
		// note that the newly calculated rating values are stored in a
		// "working" area in the Rating object
		// this avoids us attempting to calculate subsequent participants'
		// ratings against a moving target
	...
	private double v(Rating player, List<Result> results) {
		...
		for (Result result : results) {
			v = v + ((Math.pow(g(result.getOpponent(player).getGlicko2RatingDeviation()), 2))
					* E(player.getGlicko2Rating(), result.getOpponent(player).getGlicko2Rating(),
							result.getOpponent(player).getGlicko2RatingDeviation())
					* (1.0 - E(player.getGlicko2Rating(), result.getOpponent(player).getGlicko2Rating(),
							result.getOpponent(player).getGlicko2RatingDeviation())));
		}
	...
	private double outcomeBasedRating(Rating player, List<Result> results) {
		...
		for (Result result : results) {
			outcomeBasedRating = outcomeBasedRating
					+ (g(result.getOpponent(player).getGlicko2RatingDeviation()) * (result.getScore(player)
							- E(player.getGlicko2Rating(), result.getOpponent(player).getGlicko2Rating(),
									result.getOpponent(player).getGlicko2RatingDeviation())));
		}
	...
	/**
	 * This is the formula defined in step 6. It is also used for players who
	 * have not competed during the rating period.
	 * 
	 * @param phi
	 * @param sigma
	...
	/**
	 * Converts from the value used within the algorithm to a rating in the same
	 * range as traditional Elo et al
	 * 
	 * @param rating
	 *            in Glicko2 scale
	...
	/**
	 * Converts from a rating in the same range as traditional Elo et al to the
	 * value used within the algorithm
	 * 
	 * @param rating
	 *            in Glicko scale
	...
	/**
	 * Converts from the value used within the algorithm to a rating deviation
	...
	/**
	 * Converts from a rating deviation in the same range as traditional Elo et
	 * al to the value used within the algorithm
	 * 
	 * @param ratingDeviation
	 *            in Glicko scale
